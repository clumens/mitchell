...
static void print_module_decl_t (absyn_module_decl_t *node, unsigned int il)
{
   fprintf (out, "\n%*smodule_decl_t(%p) = {", INDENT(il), "", node);
   fprintf (out, "\n%*ssymtab = %p", INDENT(il+1), "", node->symtab);
