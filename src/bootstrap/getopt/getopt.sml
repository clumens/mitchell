...
(* Modifications by Chris Lumens <chris@bangmoney.org> - see futher comments
 * in getopt-sig.sml.
 *)
...
structure GetOpt' :> GET_OPT' = 
  ...
  (* Usage information *)
	 ...
	 *)
	  ...
	  fun get ([], nonOpts, mapping) = (mapping, List.rev nonOpts)
	    | get ("--"::rest, nonOpts, mapping) = (mapping, List.revAppend(nonOpts, rest))
		...
		fun addOpt (Opt opt, rest) = let
		  ...
		  | addOpt (NonOpt, rest) = get(rest, arg::nonOpts, mapping)
		in
		  if (SS.isPrefix "--" arg')
		    ...
		    then addOpt(shortOpt (SS.sub(arg', 1), SS.triml 2 arg', rest))
		     ...
		     of RequireOrder => (mapping, List.revAppend(nonOpts, arg::rest))
		      | Permute => get(rest, arg::nonOpts, mapping)
		    (* end case *))
		end
	  in
	    fn args => get(args, [], StringMap.empty: 'a StringMap.map)
	  end (* getOpt *)
