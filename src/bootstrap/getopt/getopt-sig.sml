...
(* Further modifications by Chris Lumens <chris@bangmoney.org>:
 *
 * - getOpt now returns a StringMap.map * string list.  This map holds all
 *   the 'a arg_descr results declared by the user, keyed on a string.  The
 *   string is the short or long option name with the leading dashes.
 * - Removed ReturnInOrder from arg_order since that doesn't make any sense
 *   with the new map return value.
 *)
...
signature GET_OPT' = 
  sig
      ...
      val getOpt : {
	      ...
	      options : 'a opt_descr list,
	      errFn : string -> unit
	    } -> string list -> ('a StringMap.map * string list)
       ...
       * and returns a (mapping from option string to arg_descr, non-options).
       *)      
